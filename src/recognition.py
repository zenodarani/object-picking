import numpy as np
import cv2
def recognition(target_path, template_path, match_threshold=0.1, contour_error = 10):
    ...
    for c in target_contours:
        ...
        if match <= match_threshold and template_contour_length - contour_error <= cv2.arcLength(c,True) <= template_contour_length + contour_error:
...
if __name__ == '__main__':
    contours, means, eigenvectors = recognition('../object_images/tapes_and_pipes_50ms.png', '../template_images/tape_template.png', match_threshold=0.1, contour_error=10)
    # contours, means, eigenvectors = recognition('../object_images/detection_tryal.png', '../template_images/almond_template.png', match_threshold=0.5, contour_error=20)
