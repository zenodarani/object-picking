...
class robot:
    ...
    def get_current_cart_pos(self):
        self.read_current_cart_position()
        return self.CurCartesianPosList[2:8]
    ...
    def write_cartesian_position(self, pose):
        """! Send cartesian coordinates to robot using X, Y, Z, W, P, R system. 
        These coordinates usually correlate to the tool end effectors position.
        @param X            X cartesian coordinate
        @param Y            Y cartesian coordinate
        ...
        @param R            Roll
        """
        self.CurCartesianPosList[2] = pose[0]
        self.CurCartesianPosList[3] = pose[1]
        self.CurCartesianPosList[4] = pose[2]
        self.CurCartesianPosList[5] = pose[3]
        self.CurCartesianPosList[6] = pose[4]
        self.CurCartesianPosList[7] = pose[5]
    ...
    def activate_gripper(self):
        FANUCethernetipDriver.writeR_Register(self.robot_IP, 15, 1)
        time.sleep(2.0)

    def move_gripper(self,pos, vel=255, eff=255):
        FANUCethernetipDriver.writeR_Register(self.robot_IP, 11, pos)
        FANUCethernetipDriver.writeR_Register(self.robot_IP, 12, vel)
        FANUCethernetipDriver.writeR_Register(self.robot_IP, 13, eff)
        FANUCethernetipDriver.writeR_Register(self.robot_IP, 10, 1)
    ...
    # SB new functions for bachelor course
    def move_to_pose(self, pose, speed=None):
        """! moves the robot to the desired pose
        @param  pose     (x,y,z,w,p,r) array
        @param  speed    (optional) speed in mm/s
        """
        if speed is not None:
            self.set_speed(speed)
        self.write_cartesian_position(pose)
        self.start_robot(blocking=True)

    def gripper_close(self):
        FANUCethernetipDriver.writeR_Register(self.robot_IP, self.start_register, 3)

    def gripper_open(self):
        FANUCethernetipDriver.writeR_Register(self.robot_IP, self.start_register, 4)
